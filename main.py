...
def main():
    ...
    parser.add_argument('--impl', help="(Faster)Custom op use:'cuda'; (Slower)Tensorflow op use:'ref'", type=str, default='ref')
    parser.add_argument('--config', help="Model's config be one of: 'e', 'f'", type=str, default='f')
    parser.add_argument('--dataset_name', help="Specific dataset be one of: 'ffhq', 'afhq', 'custom'", type=str, default='afhq')
    ...
    parser.add_argument('--mode', help="Inference mode be one of: 'example', 'gif', 'mixing'", type=str, default='example')
    args = parser.parse_args()


    # Validate arguments
    if args.command == 'train':
        assert args.impl.lower() in ['cuda','ref'], "Error: --impl must be one of: 'cuda', 'ref'."
        ...
        assert args.mode.lower() in ['example','gif','mixing'], "Error: Inference mode needs to be one of: 'example', 'gif, 'mixing'."
        assert args.res > 4
